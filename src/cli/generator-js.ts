import fs from 'fs';
import { AstNode, CompositeGeneratorNode, NL, processGeneratorNode, reflection } from 'langium';
import { JSElements, JSFunction, Popup, SimpleUi, SimpleUiAstType } from '../language-server/generated/ast';
...
const functionFunc = (functionEL: AstNode) => {
    ...
    fileNode.indent(functioncontent => {
        generateFunc(el.content, functioncontent);
    });
    fileNode.append('}', NL);
    return fileNode
}
...
export function generateFunc(model: JSElements, bodyNode: CompositeGeneratorNode) {
    const suiTypes = reflection.getAllTypes();
    model.jsfunction.forEach(el => {
        suiTypes.forEach(suiType => {
            const t = suiType as SimpleUiAstType;
            const isInstance = reflection.isInstance(el, t);
            if (isInstance) {
